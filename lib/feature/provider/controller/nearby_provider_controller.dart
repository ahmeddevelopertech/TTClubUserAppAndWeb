...
class NearbyProviderController extends GetxController implements GetxService {
    ...
    if (offset != 1 || _providerModel == null || reload) {
      ...
      if (offset == 1) {
        await DataSyncHelper.fetchAndSyncData(
          fetchFromLocal: () =>
              providerBookingRepo.getProviderList<CacheResponseData>(
                offset,
                body,
          ...
          onResponse: (data, source) {
            ...
            _providerList = _providerList!
                .where((provider) => !_isCompanyProvider(provider))
                .toList();
        ...
        if (response.response.statusCode == 200) {
          ...
          if (_providerModel != null) {
            _providerList!.addAll(
              ProviderModel.fromJson(response.response.body).content?.data ??
                  [],
            );
            _providerList = _providerList!
                .where((provider) => !_isCompanyProvider(provider))
  ...
  bool _isCompanyProvider(ProviderData provider) {
    final bag = <String>[
      provider.providerType ?? '',
      provider.providerSubType ?? '',
      provider.providerCategory ?? '',
      provider.owner?.userType ?? '',
    ].map((e) => e.trim().toLowerCase()).where((e) => e.isNotEmpty).join(' | ');

    if (bag.isEmpty) return false;

    return bag.contains('company') ||
        bag.contains('companies') ||
        bag.contains('companys') ||
        bag.contains('شركة') ||
        bag.contains('شركات') ||
        bag.contains('corporate') ||
        bag.contains('firm');
  }
  ...
  void _sortProviderListAndInitMap({LatLng? initialPosition}) {
    _providerList?.forEach((element) {
      double distance =
          MapHelper.getDistanceBetweenUserCurrentLocationAndProvider(
  ...
  Future<void> updateIsFavoriteStatus({
    ...
    updateIsFavoriteValue(
      _providerList?[index].isFavorite == 1 ? 0 : 1,
      providerId,
    );
    update();
    Response response = await providerBookingRepo.updateIsFavoriteStatus(
    ...
    if (response.statusCode == 200 &&
        (response.body['response_code'] == "provider_favorite_store_200" ||
            response.body['response_code'] == "provider_remove_favorite_200")) {
  ...
  void updateIsFavoriteValue(
    int status,
    String providerId, {
    bool shouldUpdate = false,
    bool fromExploreProviderScreen = true,
  }) {
    int? index = _providerList?.indexWhere(
  ...
  Future<void> setMarker(
    ...
    final BitmapDescriptor selectedProvider = await convertAssetToUnit8List(
      Images.selectedProvider,
      width: 70,
      height: 70,
    );
    final BitmapDescriptor unselectedProvider = await convertAssetToUnit8List(
      Images.selectedProvider,
      width: 60,
      height: 60,
    );
      ...
      if (_providerList![index].coordinates != null) {
        markers.add(
          Marker(
            ...
            position: LatLng(
              _providerList![index].coordinates!.latitude!,
              _providerList![index].coordinates!.longitude!,
            ),
  ...
  void _resetMarker(
    ...
    GoogleMapController mapController,
    LatLng initialPosition,
    BitmapDescriptor selectedProvider,
    BitmapDescriptor unselectedProvider,
  ...
  Future<BitmapDescriptor> convertAssetToUnit8List(
    String imagePath, {
    double height = 50,
    double width = 50,
  }) async {
