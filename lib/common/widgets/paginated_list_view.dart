...
class PaginatedListView extends StatefulWidget {
  ...
  const PaginatedListView({
    ...
    required this.offset, required this.itemView, this.bottomPadding = Dimensions.paddingSizeExtraLarge * 2, this.limit = 10,
  }) ;
...
class _PaginatedListViewState extends State<PaginatedListView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      (ResponsiveHelper.isDesktop(context) && (widget.totalSize == null || _offset >= (widget.totalSize! / (widget.limit ?? 10)).ceil() || _offsetList!.contains(_offset+1))) ?
      const SizedBox() :
